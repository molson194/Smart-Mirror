...
#define LINK_PAGE_NUMBER_INDEX = 67
#define LINK_USER_ID_INDEX = 79
...
char firefoxCall[] = "sudo -u $SUDO_USER firefox localhost:8000/Desktop/Smart-Mirror/page0.html?user=0 &";
	...
	while(!terminateCode) {
		...
		if (webPage == '0') {
			if (pwInd == 6)
			{
				...
				if((millis() - timePasswordInputReceived)>5000) {
			...
			while(digitalRead(BUTTON_CENTER)==1) {
				...
				if (digitalRead(BUTTON_CENTER)==1) { // ignore if pressed less than 30 ms
					while (digitalRead(BUTTON_CENTER)==1) {
						// wait for button release
						delay(BUTTON_WAIT); // depends on our specs
						if((millis()-timeButtonCenterPressed)>5000) {
							break;
						}
					}
					// button released
