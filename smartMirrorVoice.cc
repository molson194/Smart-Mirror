...
int webPage; // current web app page (Welcome '0', Weather 1', Youtube '2', or Calendar '3')
...
void hotwordDetected(int hotword) {
  if (hotword == 1) {
    // page 1
    goToURL(1, currentUser);
  } else if (hotword == 2) {
    // page 2
    goToURL(2, currentUser);

  } else if (hotword == 3) {
    // page 3
    goToURL(3, currentUser);
...
// Voice Thread
//todo: maybe combine with btnCenterThread
PI_THREAD(voiceThread) {
  ...
  std::string model_filename = "resources/snowboy.umdl,resources/alexa.umdl";
  std::string sensitivity_str = "0.5,0.5";
  float audio_gain = 1;
